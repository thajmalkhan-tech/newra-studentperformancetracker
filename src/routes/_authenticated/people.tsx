...
function People() {
  ...
  return (
    <AppShell>
      <PageHeader
        title="People"
        subtitle={isAdmin ? "Manage users, roles, and student links." : "The people who can view your progress."}
      />
      <div className="p-6 pb-24 md:pb-6 space-y-6">
        <MyLinks fetcher={linked} />
        {isAdmin && <AdminPanel listUsers={listUsers} assign={assign} link={link} unlink={unlink} listLinks={listLinks} qc={qc} />}
      </div>
    </AppShell>
  );
}

function MyLinks({ fetcher }: { fetcher: (a: unknown) => Promise<Array<{student_id: string; relation: string; profiles: {full_name: string | null; avatar_url: string | null} | null}>> }) {
  const { data } = useQuery({ queryKey: ["my-links"], queryFn: () => fetcher({}) });
  return (
      ...
      <ul className="divide-y divide-border">
        {(data ?? []).map((l) => (
          <li key={l.student_id} className="flex items-center justify-between py-2 text-sm">
            <span>{l.profiles?.full_name ?? l.student_id}</span>
            <span className="text-xs text-muted-foreground">{l.relation}</span>
          </li>
        ))}
        {(!data || data.length === 0) && <li className="py-6 text-center text-sm text-muted-foreground">No linked students. Ask an admin to link you.</li>}
      </ul>
    </section>
  );
}

function AdminPanel({ listUsers, assign, link, unlink, listLinks, qc }: {
  listUsers: (a: unknown) => Promise<Array<{user_id: string; full_name: string | null; roles: string[]}>>;
  assign: (a: {data: {user_id: string; role: "student"|"parent"|"faculty"|"admin"; grant: boolean}}) => Promise<unknown>;
  link: (a: {data: {student_id: string; viewer_id: string; relation: "parent"|"faculty"}}) => Promise<unknown>;
  unlink: (a: {data: {id: string}}) => Promise<unknown>;
  listLinks: (a: unknown) => Promise<Array<{id: string; relation: string; student_id: string; viewer_id: string}>>;
  qc: ReturnType<typeof useQueryClient>;
}) {
  const { data: users } = useQuery({ queryKey: ["admin-users"], queryFn: () => listUsers({}) });
  const { data: links } = useQuery({ queryKey: ["admin-links"], queryFn: () => listLinks({}) });

  const setRole = useMutation({
    mutationFn: async (v: {user_id: string; role: "student"|"parent"|"faculty"|"admin"; grant: boolean}) => assign({ data: v }),
              ...
              <div className="flex flex-wrap gap-1.5">
                {ROLES.map((r) => {
                  ...
                  return (
                    <button
                      key={r}
                      onClick={() => setRole.mutate({ user_id: u.user_id, role: r, grant: !has })}
