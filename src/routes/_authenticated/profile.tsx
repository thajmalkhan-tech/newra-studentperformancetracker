  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: p } = await supabase
        .from("profiles")
        .select("full_name, avatar_url, bio, phone, date_of_birth, institution, program, year_of_study, location")
      ...
      <div className="p-6 pb-24 md:pb-6">
        ...
        >
          ...
          <section className="rounded-2xl border border-border bg-card p-5 space-y-4">
            ...
            <Field label="Bio">
              <textarea value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} rows={4} className={input} placeholder="A short intro about yourself" />
            </Field>
          </section>
