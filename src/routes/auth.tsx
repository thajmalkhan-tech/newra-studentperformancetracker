    ...
    <div className="grid min-h-screen md:grid-cols-2">
      <div className="hidden bg-gradient-to-br from-primary/90 to-primary p-12 text-primary-foreground md:flex md:flex-col md:justify-between">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="text-lg font-semibold font-display">NEWRA</span>
        </Link>
        <div>
        ...
        <p className="text-xs opacity-70">© NEWRA</p>
      </div>
        ...
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="w-full rounded-md border border-input bg-background px-3 py-2 pr-10 text-sm outline-none focus:ring-2 focus:ring-ring" />
              <button
                type="button"
          ...
          <p className="mt-4 text-center text-sm text-muted-foreground">
            {mode === "sign-in" ? "New to NEWRA?" : "Already have an account?"}{" "}
